...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    if (currentLevel >= 3) {
      return NextResponse.json({ status: false, message: "you're already at the top, anon" });
    }
    ...
    const previewImageUrl = `${process.env.NEXT_PUBLIC_IRYS_GATEWAY}/${process.env.NEXT_PUBLIC_BASE_NFT_MANIFEST_ID}/${nftNames[nextLevel - 1]}`;
    ...
    const receipt = await uploadMetadata(newMetadata, currentURI.split("/").pop() as string);
